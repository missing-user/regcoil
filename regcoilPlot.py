...
print(
    "single_valued_current_potential_thetazeta.shape:) ",
    single_valued_current_potential_thetazeta.shape,
)
...
if np.max(np.abs(lambdas)) < 1.0e-200:
    print(
        "lambda array appears to be all 0. Changing it to all 1 to avoid a python error."
    )
    lambdas += 1
...
single_valued_current_potential_thetazeta = single_valued_current_potential_thetazeta[
    permutation, :, :
]
K2 = K2[permutation, :, :]
...
for whichPlot in range(4):
    ...
    plt.plot(
        R_slice_coil[whichPlot, :], Z_slice_coil[whichPlot, :], "b.-", label="coil"
    )
    plt.plot(
...
plt.figtext(
    0.5,
    0.995,
    "Blue dots indicate the points in the lambda scan that are plotted in later figures",
    horizontalalignment="center",
...
for whichPlot in range(numPlots):
    ...
    plt.title(
        "lambda=" + str(lambdas[ilambda_to_plot[whichPlot] - 1]), fontsize="x-small"
    )
...
if abs(x) > np.finfo(float).eps:  # if net_poloidal_current_Ampere presented
    contours = np.linspace(
        -0.5 * x, 1.5 * x, coilsPerHalfPeriod * 2 * 2, endpoint=False
    )
else:
...
for whichPlot in range(numPlots):
    ...
    data_extended = np.concatenate(
        (data, data[0:1, :]), axis=0
    )  # Add the repeated point in theta
    data_extended = np.concatenate(
        (data_extended, data_extended[:, 0:1] + x), axis=1
    )  # Add the repeated point in zeta +G
...
for whichPlot in range(numPlots):
    ...
    plt.contourf(
        zeta_coil,
        theta_coil,
        np.sqrt(np.transpose(K2[ilambda_to_plot[whichPlot] - 1, :, :])),
        numContours,
    )
    plt.colorbar()
    plt.xlabel("zeta", fontsize="x-small")
...
# Plot Bnormal middle
########################################################
print(Bnormal_total_middle.shape)
...
# plt.imshow(np.transpose(Bnormal_total_middle))
plt.colorbar()
plt.title("Bnormal_total_middle")
...
plt.subplot(numRows, numCols, 1)
plt.contourf(
    zeta_plasma, theta_plasma, np.transpose(Bnormal_from_plasma_current), numContours
)
plt.colorbar()
plt.xlabel("zeta", fontsize="x-small")
plt.ylabel("theta", fontsize="x-small")
...
plt.subplot(numRows, numCols, 2)
plt.contourf(
    zeta_plasma, theta_plasma, np.transpose(Bnormal_from_net_coil_currents), numContours
)
plt.colorbar()
plt.xlabel("zeta", fontsize="x-small")
plt.ylabel("theta", fontsize="x-small")
...
for whichPlot in range(numPlots - 2):
    ...
    plt.contourf(
        zeta_plasma,
        theta_plasma,
        np.transpose(Bnormal_total[ilambda_to_plot[whichPlot] - 1, :, :]),
        numContours,
    )
    plt.colorbar()
    plt.xlabel("zeta", fontsize="x-small")
    plt.ylabel("theta", fontsize="x-small")
    plt.title(
        "Total, lambda=" + str(lambdas[ilambda_to_plot[whichPlot] - 1]),
        fontsize="x-small",
    )
...
ax.plot_surface(
    r_coil[minIndex:maxIndex, :, 0],
    r_coil[minIndex:maxIndex, :, 1],
    r_coil[minIndex:maxIndex, :, 2],
    rstride=1,
    cstride=1,
    color="b",
    linewidth=0,
